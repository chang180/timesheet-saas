...
Phase 3 已標示完成（見 `phase-3.md`）。以下為進入 **Phase 4（部署／CI/CD／維運）** 前的現況勾選：

- **高優先級（已完成）**：假期同步、匯總報表（公司／單位／部門／小組）、報表匯出（CSV／XLSX）。
- **中優先級（已完成）**：IP 白名單 middleware、審計日誌、通知與提醒（排程內建）。
- **測試**：以本機執行 `php artisan test --compact` 為準；測試數量隨專案演進變動，驗證報告見 `phase-3-verification-report.md`。

**尚未滿足、仍屬 Phase 4 範疇者**：

- 正式環境 **CI/CD** pipeline（例如 GitHub Actions）、**staging／production** 一鍵部署與 artifact。
- **監控與備援**（日誌匯流、告警、資料庫備份策略等）。

完成上述營運面項目後，再將本文件完成度標為進行中／完成。
