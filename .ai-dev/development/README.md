...
- `phase-3.md`：整合測試、通知機制、報表匯出與安全控制。（約 5% 完成）
...
- 設定：歡迎頁、IP 白名單（UI）、租戶品牌。
- 測試：約 26 個 Feature Tests、2 個 Browser Tests。

### 下一步建議

1. 優先完成 Phase 3 高優先級：假期同步、匯總報表、報表匯出（CSV/XLSX）。
2. 補齊 Phase 3 中優先級：IP 白名單 middleware、審計日誌記錄、通知與提醒。
3. 視需求規劃 HQ Portal、Redmine/Jira 整合（可選）。
