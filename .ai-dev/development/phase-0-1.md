...
**說明**：認證與租戶基礎、組織與成員 API、歡迎頁與 IP 白名單、HQ Portal 均已完成。

---

## 歷史指示（已封存）

本文件曾含「HQ Portal 未完成」之 AI 工作指示，與現況不符（HQ 已於 2026-01-30 完成，見第 3.2 節與文末「2026-01-30 更新」）。該段全文已移至 [.ai-dev/archive/development/phase-0-1-superseded-hq-instructions-2026-03-25.md](../archive/development/phase-0-1-superseded-hq-instructions-2026-03-25.md)。封存規則見 [.ai-dev/archive/README.md](../archive/README.md)。
...
- ✅ HQ Portal：已於 2026-01-30 實作完成。以獨立 Route Prefix `api/v1/hq` 並限制 `hq_admin` 角色；提供公司 CRUD 與 user_limit 管理 API。
...
- ✅ HQ Portal：已於 2026-01-30 完成實作。

### 2026-01-30 更新（HQ Portal 實作完成）

- ✅ HQ Portal API 實作完成，包含：
  - `GET /api/v1/hq/companies` - 列出所有公司（支援分頁）
  - `POST /api/v1/hq/companies` - 建立公司（含 slug 唯一性驗證、可選指定管理者）
  - `GET /api/v1/hq/companies/{company}` - 取得單一公司詳情
  - `PATCH /api/v1/hq/companies/{company}` - 更新公司（status、user_limit、branding 等，slug 不可變）
  - `PATCH /api/v1/hq/companies/{company}/user-limit` - 僅更新 user_limit
- ✅ `EnsureHqAdmin` Middleware：僅 `hq_admin` 角色可存取 HQ 端點，非授權用戶回傳 403
- ✅ FormRequest 驗證：`StoreCompanyRequest`、`UpdateCompanyRequest` 提供完整欄位驗證
- ✅ Feature Test：`tests/Feature/Hq/CompanyManagementTest.php` 涵蓋 22 項測試，包含授權檢查、CRUD 操作、驗證規則等
