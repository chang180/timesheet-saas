...
export default function WeeklyReports({ isManager, tenant }: WeeklyProps) {
    const needsOnboarding = Boolean(
        isManager && tenant && tenant.onboarded === false,
    );
            ...
            <div className="flex flex-col gap-8">
                ...
                {needsOnboarding && (
                    <div className="rounded-2xl border-2 border-amber-300 bg-linear-to-r from-amber-50/90 to-amber-100/60 p-6 shadow-md dark:border-amber-600 dark:from-amber-950/30 dark:to-amber-900/20">
                        <p className="text-base font-bold text-amber-900 sm:text-lg dark:text-amber-200">
                            第一次使用週報通？
                        </p>
                        <p className="mt-2 text-sm text-amber-800 sm:text-base dark:text-amber-300">
                ...
                <section className="grid gap-6 sm:grid-cols-2">
                    <div className="group flex flex-col rounded-2xl border-2 border-border/60 bg-card p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg">
                        ...
                        <Link
                            href="#"
                            className="mt-auto inline-flex items-center gap-2 self-start rounded-full bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-indigo-500 hover:shadow-lg focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
                        >
                            前往填寫
